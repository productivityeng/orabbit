...
describe("UserTable render state", () => {
  ...
  it("should render the component with all actions buttons disabled by default", async () => {
    const { findByTestId } = render(
      <UserTable data={users} columns={RabbitMqUserTableColumnsDef} />
    );
  ...
  it("should sync user when sync button is clicked", async () => {
    ...
    const { findByTestId } = render(
      <UserTableContext.Provider
        value={{
          onSyncronizeUser: SyncronizeUserActionMock,
  ...
  it("should remove user when remove button is clicked", async () => {
    ...
    const { findByTestId } = render(
      <UserTableContext.Provider
        value={{
          onRemoveUser: removeUserFromClusterMock,
          onSyncronizeUser: jest.fn(),
        }}
      >
        <UserTable
          data={[
  ...
  it("should call import user when user is not tracked", async () => {
    ...
    const { findByTestId } = render(
      <UserTableContext.Provider value={{ onImportUser: onImportUserMock }}>
        <UserTable
          data={[
            {
              Id: 1,
  ...
  it("should lock user when lock button is clicked", async () => {
    ...
    const { findByTestId } = render(
      <UserTableContext.Provider value={{ onLockUser: onLockUserMock }}>
        <UserTable
          data={[
            {
              Id: 1,
    ...
    const lockItemReasonTextarea = await findByTestId(
      "lock-item-reason-textarea"
    );
    act(() => {
      fireEvent.change(lockItemReasonTextarea, {
        target: { value: "reason with more than 10 characters" },
      });
    });
  ...
  it("should unlock user when unlock button is clicked", async () => {
    ...
    const { findByTestId } = render(
      <UserTableContext.Provider value={{ onUnlockUser: onUnlockUserMock }}>
        <UserTable
          data={[
            {
              Id: 1,
