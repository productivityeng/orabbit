...
import (
	...
	functions "github.com/productivityeng/orabbit/pkg/functions/exchange"
...
func (ctrl *ExchangeController) ImportExchange(c *gin.Context)  {
	...
	if err != nil { 
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Fail to save exchange in database"})
		return 
	}
	err = functions.ImportExchangeBindings(ctrl.DependencyLocator,
		cluster,
		exchange.Name,
		virtualHost.ID,
		c,
	...
	if err != nil { 
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Fail to import exchange bindings"})
		return 
	}
