...
export async function fetchQueue(queueId: number,clusterId: number) : Promise<FrontResponse<RabbitMqQueue|null>> {
    const fetchUserEndpoint = `${process.env.PRIVATE_INVENTORY_ENDPOINT!}/${clusterId}/queue/${queueId}`
  ...
  export async function createQueue(clusterId:number,request: z.infer<typeof CreateRabbitMqQeueueRequestSchema>): Promise<FrontResponse<RabbitMqQueue | null>> {
    const createUserEndpoint = `${process.env.PRIVATE_INVENTORY_ENDPOINT!}/${clusterId}/queue`
